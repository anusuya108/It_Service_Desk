# app.py — Streamlit web UI (fully fixed)
...
.stButton > button { background:#3b82f6 !important; color:white !important; border:none !important; border-radius:8px !important; width:100% !important; font-weight:500 !important; padding:0.6rem 2rem !important; }
...
.agent-content { color:#cbd5e1; font-size:0.9rem; line-height:1.7; }
.reasoning-box { background:#0d1520; border:1px solid #1e2d40; border-radius:8px; padding:10px 12px; margin-top:10px; font-size:0.82rem; color:#64748b; font-style:italic; }
...
.badge-method  { background:#1a1a2e; color:#818cf8; border:1px solid #3730a3; font-size:0.68rem; }
.metrics-row { display:flex; gap:10px; margin:1.2rem 0; flex-wrap:wrap; }
.metric-box  { flex:1; min-width:80px; background:#161b27; border:1px solid #1e2d40; border-radius:10px; padding:0.9rem; text-align:center; }
.metric-val  { font-family:'JetBrains Mono',monospace; font-size:1.2rem; font-weight:500; color:#f1f5f9; }
.metric-lbl  { font-size:0.72rem; color:#475569; margin-top:3px; }
...
samples = {
    ...
    "Ransomware message on screen":    "I clicked a suspicious link and now see a ransom message on screen.",
    "Monitor flickering":              "My monitor keeps flickering and sometimes goes completely black.",
    "Strange login detected":          "Someone is logged in as me on a machine I don't recognise.",
    "Teams crashes on every meeting":  "Microsoft Teams crashes every time I try to join a meeting.",
...
if submit:
    if not user_issue.strip():
        ...
        with st.spinner("Running 3 agents — triage → resolution → escalation..."):
            try:
                result = process_ticket(ticket_id, user_issue)
            except Exception:
                # Never show raw errors to user
                st.error("Something went wrong processing your ticket. Please try again.")
                st.stop()

        # Safe defaults — never crash on missing keys
        result.setdefault("category",             "other")
        result.setdefault("priority",             "medium")
        result.setdefault("classification_method","rule-based")
        result.setdefault("triage_reasoning",     "")
        result.setdefault("resolution",           "Please contact the IT helpdesk directly.")
        ...
        method_badge = result['classification_method'].replace(" ","&nbsp;")
        reasoning    = result['triage_reasoning'] or "Classification complete."
        st.markdown(f"""
        ...
        res_label = "Agent 2 — Resolution (escalation triggered)" if result["escalated"] else "Agent 2 — Resolution (resolved at L1)"
        res_html  = result["resolution"].replace("\n","<br>")
        st.markdown(f"""
        ...
        if result["escalated"]:
            ...
            <div class="agent-card escalation">
                ...
                <div class="agent-content" style="font-family:'JetBrains Mono',monospace;font-size:0.8rem;line-height:1.8;">
                    {report_html}
                </div>
            </div>
            """, unsafe_allow_html=True)
...
if st.session_state.history:
    ...
    for item in st.session_state.history[:10]:
        ...
        <div style="background:#161b27;border:1px solid #1e2d40;border-radius:8px;
            ...
            <span style="flex:1;font-size:0.84rem;color:#94a3b8;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{item['issue'][:70]}</span>
